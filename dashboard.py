...
# --- Add src to path (guarded against double-insertion) ---
...
try:
    ...
    from config import MODEL_PATH, EVAL_FEDERATED_JSON, EVAL_LOCAL_JSON, PLOT_ROC_PATH, PLOT_PRC_PATH, N_FEATURES, SEQ_LEN, SCALER_PATH
except ImportError:
    ...
    st.info("Please activate your project's virtual environment and run `pip install -r requirements.txt`")
    st.stop()
...
@st.cache_data
def preprocess_data(df, seq_len, n_features, scaler=None):
    """Validates, pads/truncates, optionally normalises, and converts to tensor."""
    try:
    ...
    if page == "Live Prediction":
        ...
        if uploaded_file is not None:
            try:
                ...
                tensor_data, message = preprocess_data(df, SEQ_LEN, N_FEATURES, scaler=scaler)
                
                if tensor_data is not None:
                    if st.button("🔍 Predict Sepsis Risk", use_container_width=True):
                            ...
                            with col2:
                                ...
                                if probability > 0.5:
                                    ...
                                    **Clinical Recommendations:**
                                    ...
                                    st.success("Low Risk: Sepsis unlikely.", icon="✅")
                                    st.markdown("""
                                    **Clinical Recommendations:**
